...
export function HomePageV7({ onCreate, onOpen, onImport }: HomePageV7Props) {
  ...
  return (
    <div className="relative flex h-screen bg-background">
      ...
      <main className="relative z-10 flex flex-1 overflow-hidden">
        {/* 左栏：品牌 + 操作（占 50%） */}
        <div className="flex flex-1 flex-col px-12 py-12 md:px-16 lg:px-20">
          {/* 顶部：品牌区 */}
          <motion.div
          ...
          >
            <div className="mb-5 flex items-center gap-3">
              ...
              <div className="flex flex-col">
                <span className="text-sm font-semibold tracking-wider text-foreground">NEXCUBE</span>
                <span className="text-[10px] text-muted-foreground/60">MC Mod IDE</span>
              </div>
            </div>
          ...
          >
            <motion.button
              ...
              whileHover={{ y: -2 }}
              whileTap={{ scale: 0.98 }}
              className="group flex items-center gap-2 rounded-xl bg-gradient-brand px-5 py-3 shadow-glow transition-shadow hover:shadow-glow-strong"
            >
              <Plus className="h-4 w-4 text-primary-foreground" />
              <span className="text-sm font-medium text-primary-foreground">创建新项目</span>
            ...
            <motion.button
              ...
              whileHover={{ y: -2 }}
              whileTap={{ scale: 0.98 }}
              className="flex items-center gap-2 rounded-xl border border-border/50 bg-card/30 px-5 py-3 transition-colors hover:border-border/80 hover:bg-card/50"
            >
              <Download className="h-4 w-4 text-cyan-400" />
              <span className="text-sm font-medium text-foreground">导入</span>
            </motion.button>
          </motion.div>
        ...
        <motion.div
          ...
          className="flex w-[440px] shrink-0 flex-col border-l border-border/30 bg-sidebar/10 md:w-[520px]"
        >
          {/* 标题栏 */}
          ...
          {/* 项目列表 */}
          <div className="nexcube-scroll flex-1 overflow-y-auto px-4 py-3">
            {projects.length === 0 ? (
...
function RecentItem({
  ...
  return (
    <motion.button
      ...
      className={cn(
        'group flex items-center gap-3 rounded-lg border border-transparent px-3 py-2.5 text-left transition-all duration-200',
