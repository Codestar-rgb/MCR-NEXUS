...
export function HomePageV6({ onCreate, onOpen, onImport }: HomePageV6Props) {
    ...
    <div className="relative flex h-screen bg-background">
      ...
      <main className="relative z-10 flex flex-1 overflow-hidden">
        ...
        <div className="flex flex-1 flex-col justify-center px-12 py-12 md:px-16 lg:px-20">
          ...
          >
            ...
            <motion.button
              ...
              whileHover={{ y: -2 }}
              whileTap={{ scale: 0.98 }}
              className="group relative flex items-center gap-3 overflow-hidden rounded-xl bg-gradient-brand px-5 py-3.5 text-left shadow-glow transition-shadow hover:shadow-glow-strong"
            >
              <div className="flex h-8 w-8 items-center justify-center rounded-lg bg-white/15">
                <Plus className="h-4 w-4 text-white" />
              </div>
              <div className="flex-1">
                <div className="text-sm font-semibold text-primary-foreground">创建新项目</div>
                <div className="text-[11px] text-primary-foreground/70">从零开始，引导式创建</div>
              </div>
              <ArrowRight className="h-4 w-4 text-primary-foreground/70 transition-transform group-hover:translate-x-0.5" />
            </motion.button>
            ...
            <motion.button
              ...
              whileHover={{ y: -1 }}
              whileTap={{ scale: 0.98 }}
              className="group flex items-center gap-3 rounded-xl border border-border/50 bg-card/30 px-5 py-3 text-left transition-colors hover:border-border/80 hover:bg-card/50"
            >
        ...
        <motion.div
          ...
          className="flex w-[400px] shrink-0 flex-col border-l border-border/30 bg-sidebar/10 px-6 py-12 md:w-[460px]"
        >
          {/* 标题 */}
          <div className="mb-5 flex items-center justify-between">
            ...
            {projects.length > 0 && (
              <span className="text-[11px] text-muted-foreground/50">{projects.length} 个</span>
            )}
          </div>
...
function EmptyState({ onCreate }: { onCreate: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center gap-4 py-16 text-center">
      <div className="flex h-14 w-14 items-center justify-center rounded-2xl border border-dashed border-border/50 text-muted-foreground/50">
...
function RecentItem({
  ...
  return (
    <motion.button
      ...
      className={cn(
        'group flex items-center gap-3 rounded-lg border border-border/30 bg-card/30 p-3 text-left transition-all duration-200',
